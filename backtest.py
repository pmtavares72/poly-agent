"""
POLYMARKET BOND HUNTER — BACKTESTING SCRIPT
============================================
...
USO
----
python backtest.py                             # parámetros por defecto
python backtest.py --days 60 --capital 500    # personalizar
python backtest.py --min-prob 0.93 --max-prob 0.99
"""
...
def init_db(db_path: str = "backtest.db") -> sqlite3.Connection:
...
def run_backtest(params: dict):
    ...
    console.print(f"\n[dim]Resultados guardados en [bold]backtest.db[/bold] (run_id={run_id})[/dim]\n")
...
def run_paper(params: dict):
    ...
    console.print(f"  Datos guardados en:        [bold]backtest.db[/bold] → tabla [bold]signals[/bold]\n")
...
if __name__ == "__main__":
    ...
    if mode == "paper":
        ...
        console.print(f"  Capital inicial: [bold]${params['initial_capital']:.2f}[/bold]  |  "
                      f"Período: [bold]{params['days_back']} días[/bold]  |  "
                      f"Prob. entrada: [bold]{params['min_prob']}-{params['max_prob']}[/bold]\n")
        run_backtest(params)

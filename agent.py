...
def resolve_pending_signals(conn: sqlite3.Connection) -> int:
    """
    Para cada señal en estado 'open' cuyo closes_at ya pasó, consulta el
    CLOB last-trade-price del token YES para determinar el outcome:
      - precio >= 0.9  → resuelto YES (ganamos)
      - precio <= 0.1  → resuelto NO  (perdemos)
      - precio en medio → aún no resuelto; expirar tras 6h
    Nota: el Gamma API filter por clobTokenIds no funciona como filtro real
    (devuelve resultados aleatorios), por eso usamos directamente el CLOB.
    """
    cur = conn.cursor()
    ...
    for row in pending:
        ...
        # Query CLOB last-trade-price for the YES token directly by token_id
        ...
        # YES token resolves near 1.0 (winner) or near 0.0 (loser)
        if last_price >= 0.9:
            outcome = "YES"
            ...
            # Price in 0.1–0.9 range: market not yet settled — expire after 6h
            if hours_past > 6:
                cur.execute("UPDATE signals SET status='expired' WHERE id=?", (sig_id,))
                conn.commit()
            continue
